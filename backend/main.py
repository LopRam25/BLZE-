...
@app.post("/api/admin/import_supplier")
async def import_supplier(admin: bool = Depends(verify_admin)):
    """Fetch the supplier menu page, scrape links, and add any new products."""
    try:
        resp = requests.get(
            SUPPLIER_MENU_URL,
            timeout=20,
            headers={
                "User-Agent": "Mozilla/5.0 (compatible; BLZEBot/1.0; +https://blze-cannabis.example)"
            },
        )
        ...
        for anchor in soup.find_all("a"):
            ...
            # crude filter: links that contain '/product/' or 'thca' words
            if "product" not in href and "thca" not in href:
                continue
            ...
            product = {
                ...
                "pricing": {},
                "quantity": 0,
                "description": f"Imported from supplier on {dt.utcnow().isoformat()}",
                "images": [],
                "isPremium": False,
                "isVisible": True
            }
