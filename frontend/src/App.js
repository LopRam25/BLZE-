...
const CartModal = ({ isOpen, onClose, cart, updateCart, removeFromCart, deliveryLocation }) => {
  ...
  const handlePayment = async (method) => {
    ...
    const orderData = {
      customer: {
        name: 'Customer', // Placeholder for customer name
        phone: customerPhone,
        email: customerEmail
      },
      items: cart,
          ...
          <div className="flex flex-wrap justify-center gap-1 text-xs">
            {product.pricing ? (
              Object.entries(product.pricing).map(([weight, price]) => (
                <div key={weight} className="flex justify-between text-sm items-center">
                  <span className="font-medium">{weight}</span>
                  <span className="bg-blue-600 text-white px-2 py-1 rounded-md font-bold">${price}</span>
                </div>
              ))
            ) : (
